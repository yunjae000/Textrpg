...
namespace TextRpg
{
    internal class Program
    {
        ...
        { //시작 화면
            ...
            // 아이템 생성
            Item TrainingArmor = new Item("수련자 갑옷  ", 0, 5, 0, 0, "수련에 도움을 주는 갑옷입니다.   ", 1000);
            Item SteelArmor = new Item("무쇠 갑옷  ", 0, 9, 0, 0, "수련에 도움을 주는 갑옷입니다.   ", 2300);
            Item SpartanArmor = new Item("스파르타의 갑옷  ", 0, 15, 0, 0, "수련에 도움을 주는 갑옷입니다.   ", 3500);
            Item RustySword = new Item("낡은 검  ", 2, 0, 0, 0, "수련에 도움을 주는 갑옷입니다.   ", 600);
            Item BronzeAxe = new Item("청동 도끼  ", 5, 0, 0, 0, "수련에 도움을 주는 갑옷입니다.   ", 1500);
            Item SpartanSpear = new Item("스파르타의 창  ", 7, 0, 0, 0, "수련에 도움을 주는 갑옷입니다.   ", 3700);
            Item LifeRuby = new Item("생명의 루비 펜던트", 0, 0, 50, 0, "최대 체력을 상승시켜주는 신비한 펜던트입니다.", 4000);
            Item LichMark = new Item("부의 상징 ◐", 0, 0, 0, 5000, " 추가 골드 획득", 50);
        ...
        {
            string attackInfo = item.ItemAttack > 0 ? $"공격력: {item.ItemAttack} " : "";
            string defenseInfo = item.ItemDefense > 0 ? $"방어력: {item.ItemDefense} " : "";
            string hpinfo = item.ItemHp > 0 ? $"HP: {item.ItemHp} " : "";
            string goldinfo = item.Itemgold > 0 ? $"+ Gold: {item.Itemgold}" : "";

            Console.WriteLine($"{item.Name} | {attackInfo}{defenseInfo}| {item.Description}");
        }
    ...
    class Item
     {
        ...
        public Item (string name, int attack, int defense,int itemhp, int itemgold, string description, int cost) //생성자 사용
        {
            Name = name;
            ItemAttack = attack;
            ItemDefense = defense;
    ...
    class Character 
    {
        ...
        public Character (string name, string job,int baseattack, int basedefense, int hp, int gold)
        {
           ...
           Gold = gold;
            ...
            public int TotalHp => HP + (EquippedItem?.ItemHp ?? 0);
             public int TotalGold => Gold + (EquippedItem?.Itemgold ?? 0);
        public void ShowStatus()
            {
                ...
                Console.WriteLine($"체력: {TotalHp} +  ({EquippedItem?.ItemHp})");
                Console.WriteLine($"Gold: {TotalGold} G");
